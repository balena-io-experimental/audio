...
const { getAddrs, getCookie, parseServerString } = require('./configuration.js');
const {
...
PAClient.prototype.connect = function(config) {
  ...
  if (typeof config === 'object' && config !== null) {
    ...
    } else if (config.serverString) {
      addrs = addrs.concat(parseServerString(config.serverString));
    }
...
PAClient.prototype.setSinkSuspend = function(criteria, suspended, cb) {
  if (typeof criteria === 'number') {
    ...
    throw new Error('Sink criteria must be a name or index');
  } else if (typeof suspended !== 'boolean') {
    throw new Error('Suspended argument must be a boolean');
  }
  if (this._index === -1)
    throw new Error('Not ready');
  ...
  buf[buf.length - 1] = (suspended ? PA_TAG_BOOLEAN_TRUE : PA_TAG_BOOLEAN_FALSE);
...
PAClient.prototype.setCardProfile = function(card, profile, cb) {
  ...
  const cmd = PA_COMMAND_SET_CARD_PROFILE;
  const buf = new PacketBuilder()
    .putu32(cmd)
    .putu32(addReq(this, cmd, cb))
    .putu32(card)
...
function parseReply(payload, cmd, serverVer) {
  ...
  switch (cmd) {
    ...
    case PA_COMMAND_GET_CARD_INFO: {
      const card = readCard(payload, 10, len, serverVer);
      if (card === undefined)
        return;
      return card[1];
    }
